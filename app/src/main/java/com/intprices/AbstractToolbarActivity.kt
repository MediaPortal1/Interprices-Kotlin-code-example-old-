...
abstract class AbstractToolbarActivity : AppCompatActivity(), AdapterView.OnItemSelectedListener {
    ...
    override fun onOptionsItemSelected(item: MenuItem?): Boolean {
        when (item?.itemId) {
            ...
            R.id.menu_filter -> openDrawer(!drawerlayout.isDrawerOpen(Gravity.RIGHT))
            else -> return false
        }
        return true
    }
    ...
    protected fun makeSearchRequest(query: String = "", page: Int = 1): Bundle {
        ...
        if (!query.isNullOrBlank()) bundle.addNotNull("query", form_home_query.text.toString())
    ...
    protected fun makeSearchMap(query: String = "", page: Int = 1) {
        requestMap = makeSearchRequest(form_home_query.text.toString(), page).loadFromBundle() as HashMap<String, String>
    }
    ...
    protected fun Bundle.hasExtra(key: String) = if (!get(key).toString().isNullOrBlank()) true else false
    ...
    private fun initOnFilterSelected() {
        ...
        settings_checkbox.setOnCheckedChangeListener { button, b -> isFreeShipping = b }
